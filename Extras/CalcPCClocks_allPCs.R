...
#Note: this code assumes all your files are in your working directory. Alter the code as needed based on file locations.
#Load packages
pkgTest <- function(x)
{
...
#Once you download the results, place the file into your working directory
...
sum(DNAmAge$SampleID == datPheno$SampleID)
sum(DNAmAge$SampleID == rownames(datMeth))

#Calculate PC Clocks
load(file = paste(clocksDir,"Extras/CalcPCHorvath1.RData",sep=""))
datPCHorvath1 = predict(PCHorvath1,datMeth)[,-4282]
DNAmAge$PCHorvath1 = as.numeric(predict.glmnet(fit,datPCHorvath1,s = cv$lambda.min))
anti.trafo= function(x,adult.age=20) { ifelse(x<0, (1+adult.age)*exp(x)-1, (1+adult.age)*x+adult.age) }
DNAmAge$PCHorvath1 = anti.trafo(DNAmAge$PCHorvath1)
load(file = paste(clocksDir,"Extras/CalcPCHorvath2.RData",sep=""))
datPCHorvath2 = predict(PCHorvath2,datMeth)
DNAmAge$PCHorvath2 = as.numeric(predict.glmnet(fit,datPCHorvath2,s = cv$lambda.min))
DNAmAge$PCHorvath2 = anti.trafo(DNAmAge$PCHorvath2)
load(file = paste(clocksDir,"Extras/CalcPCHannum.RData",sep=""))
datPCHannum = predict(PCHannum,datMeth)
DNAmAge$PCHannum = as.numeric(predict.glmnet(fit,datPCHannum,s = cv$lambda.min))
load(file = paste(clocksDir,"Extras/CalcPCPhenoAge.RData",sep=""))
datPCPhenoAge = predict(PCPhenoAge,datMeth)
DNAmAge$PCPhenoAge = as.numeric(predict.glmnet(fit,datPCPhenoAge,s = cv$lambda.min))
load(file = paste(clocksDir,"Extras/CalcPCDNAmTL.RData",sep=""))
datPCDNAmTL = predict(PCDNAmTL,datMeth)
DNAmAge$PCDNAmTL = as.numeric(predict.glmnet(fit,datPCDNAmTL,s = cv$lambda.min))
load(file = paste(clocksDir,"Extras/CalcPCGrimAge.RData",sep=""))
datPCGrimAge = predict(PCGrimAge,datMeth)[,-3935]
datPCGrimAge = cbind(datPCGrimAge,Female = datPheno$Female,Age = datPheno$Age)
for(i in 2:9){
  DNAmAge[,GrimAgeComponents[i]] = as.numeric(predict.glmnet(fit[[i]],datPCGrimAge,s=cv[[i]]$lambda.min))
}
datGrimAge = data.matrix(DNAmAge)[,c(GrimAgeComponents[2:9],"Age","Female")]
DNAmAge[,GrimAgeComponents[10]] = as.numeric(predict.glmnet(fit[[10]],datGrimAge,s=cv[[10]]$lambda.min))
rm(PCHorvath1,PCHorvath2,PCHannum,PCPhenoAge,PCGrimAge,PCDNAmTL,cv,fit,GrimAgeComponents)
